...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) { header('Location: index.php?error=Invalid program id'); exit; }

$res = mysqli_query($conn, "SELECT id, page_id, title, description, image, features FROM teaching_programs WHERE id = $id");
$row = $res ? mysqli_fetch_assoc($res) : null;
if (!$row) { header('Location: index.php?error=Program not found'); exit; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
	...
	<title>Edit Teaching Program</title>
	...
	<?php include '../includes/topbar.php'; ?>
		<main class="admin-main">
			<header class="page-header">
				<h1><i class="fas fa-graduation-cap"></i> Edit Teaching Program</h1>
			</header>
			<section class="card">
				<div class="card-body">
					<form action="handlers/programs-update.php" method="post" enctype="multipart/form-data" class="form-grid">
						...
						<div class="form-group">
							...
							<textarea name="features" rows="5"><?php echo htmlspecialchars($row['features']); ?></textarea>
						</div>
						<div class="form-group">
