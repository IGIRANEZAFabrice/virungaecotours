...
if ($id <= 0 || $page_id <= 0 || $title === '') { header('Location: ../index.php?error=Invalid input'); exit; }
...
$ok = mysqli_query($conn, "UPDATE impact_gallery SET title='$title_safe', description='$desc_safe' $image_clause WHERE id=$id AND page_id=$page_id");
header('Location: ../index.php?' . ($ok ? 'success=Gallery item updated' : 'error=Update failed'));
exit;
