...
// Get page data
$page = null;
$pq = mysqli_query($conn, "SELECT id, hero_image FROM heritage_page ORDER BY id ASC LIMIT 1");
...
// Handle image upload
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_FILES['hero_image']['name'])) {
    ...
    if (in_array($file_ext, $allowed_ext)) {
        $new_file_name = 'heritage-hero-' . time() . '.' . $file_ext;
        $upload_path = $upload_dir . $new_file_name;

        if (move_uploaded_file($_FILES['hero_image']['tmp_name'], $upload_path)) {
            $hero_image = 'assets/images/' . $new_file_name;
            $update_query = "UPDATE heritage_page SET hero_image = '" . mysqli_real_escape_string($conn, $hero_image) . "' WHERE id = $page_id";
...
<body class="admin-layout">
	...
	<div class="container">
		<h1 style="margin-bottom: 2rem;">Edit Hero Image</h1>
		...
		<div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
			<h2 style="margin-bottom: 1.5rem; font-size: 1.25rem;">Current Hero Image</h2>
