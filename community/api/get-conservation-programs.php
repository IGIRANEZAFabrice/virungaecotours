<?php
/**
 * API Endpoint: Get Conservation Programs
 * Returns all programs with category = 'Conservation'
 */
...
try {
    // Query to get conservation programs
    $query = "SELECT 
                id,
                title,
                short_description,
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $program = [
            ...
            'image' => $row['image'] ?: 'default-conservation.jpg',
        ...
        $program['conservation_focus'] = getConservationFocus($program['title'], $program['description']);
    ...
    $response = [
        ...
        'programs' => $programs,
        'message' => count($programs) > 0 ? 'Conservation programs loaded successfully' : 'No conservation programs found'
    ];
...
/**
 * Determine conservation focus areas based on program content
 */
function getConservationFocus($title, $description) {
    $focus_areas = [];
    $content = strtolower($title . ' ' . $description);
    
    // Define focus area keywords
    $focus_keywords = [
        'habitat' => ['habitat', 'ecosystem', 'forest', 'park', 'reserve'],
        'wildlife' => ['gorilla', 'wildlife', 'animal', 'species', 'biodiversity'],
        'community' => ['community', 'local', 'village', 'people', 'resident'],
        'education' => ['education', 'awareness', 'training', 'school', 'learning'],
        'research' => ['research', 'monitoring', 'study', 'data', 'science'],
        'restoration' => ['restoration', 'reforestation', 'planting', 'rehabilitation']
    ];
...
/**
 * Calculate conservation impact metrics
 */
function getConservationMetrics($programs) {
    $metrics = [
    ...
    foreach ($programs as $program) {
        ...
        if (isset($program['conservation_focus'])) {
            $metrics['focus_areas'] = array_merge($metrics['focus_areas'], $program['conservation_focus']);
        }
    }
